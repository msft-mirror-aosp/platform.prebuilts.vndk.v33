...
public:
  explicit IGnssMeasurementCallbackDelegator(const std::shared_ptr<IGnssMeasurementCallback> &impl) : _impl(impl) {
     int32_t _impl_ver = 0;
     if (!impl->getInterfaceVersion(&_impl_ver).isOk()) {;
        __assert2(__FILE__, __LINE__, __PRETTY_FUNCTION__, "Delegator failed to get version of the implementation.");
     }
     if (_impl_ver != IGnssMeasurementCallback::version) {
        __assert2(__FILE__, __LINE__, __PRETTY_FUNCTION__, "Mismatched versions of delegator and implementation is not allowed.");
     }
  }

  ::ndk::ScopedAStatus gnssMeasurementCb(const ::aidl::android::hardware::gnss::GnssData& in_data) override {
    return _impl->gnssMeasurementCb(in_data);
  }
protected:
private:
  std::shared_ptr<IGnssMeasurementCallback> _impl;
};
