...
public:
  ...
  inline bool operator!=(const RadioResponseInfoModem& rhs) const {
    return std::tie(type, serial, error, isEnabled) != std::tie(rhs.type, rhs.serial, rhs.error, rhs.isEnabled);
  }
  inline bool operator<(const RadioResponseInfoModem& rhs) const {
    return std::tie(type, serial, error, isEnabled) < std::tie(rhs.type, rhs.serial, rhs.error, rhs.isEnabled);
  }
