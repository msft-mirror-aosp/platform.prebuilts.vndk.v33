...
public:
  ...
  inline bool operator!=(const ReadbackBufferAttributes& rhs) const {
    return std::tie(format, dataspace) != std::tie(rhs.format, rhs.dataspace);
  }
  inline bool operator<(const ReadbackBufferAttributes& rhs) const {
    return std::tie(format, dataspace) < std::tie(rhs.format, rhs.dataspace);
  }
