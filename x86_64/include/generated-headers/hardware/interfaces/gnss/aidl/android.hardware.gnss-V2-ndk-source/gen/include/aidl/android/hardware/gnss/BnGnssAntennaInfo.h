...
namespace hardware {
namespace gnss {
class BnGnssAntennaInfo : public ::ndk::BnCInterface<IGnssAntennaInfo> {
public:
  BnGnssAntennaInfo();
  virtual ~BnGnssAntennaInfo();
...
public:
  explicit IGnssAntennaInfoDelegator(const std::shared_ptr<IGnssAntennaInfo> &impl) : _impl(impl) {
     int32_t _impl_ver = 0;
     if (!impl->getInterfaceVersion(&_impl_ver).isOk()) {;
        __assert2(__FILE__, __LINE__, __PRETTY_FUNCTION__, "Delegator failed to get version of the implementation.");
     }
     if (_impl_ver != IGnssAntennaInfo::version) {
...
protected:
private:
  std::shared_ptr<IGnssAntennaInfo> _impl;
};
