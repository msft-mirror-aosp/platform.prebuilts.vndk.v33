...
public:
  ...
  bool isRemoteKeyProvisioningSupported = false;

  binder_status_t readFromParcel(const AParcel* parcel);
  binder_status_t writeToParcel(AParcel* parcel) const;

  inline bool operator!=(const HardwareInformation& rhs) const {
    return std::tie(credentialStoreName, credentialStoreAuthorName, dataChunkSize, isDirectAccess, supportedDocTypes, isRemoteKeyProvisioningSupported) != std::tie(rhs.credentialStoreName, rhs.credentialStoreAuthorName, rhs.dataChunkSize, rhs.isDirectAccess, rhs.supportedDocTypes, rhs.isRemoteKeyProvisioningSupported);
  }
  inline bool operator<(const HardwareInformation& rhs) const {
    return std::tie(credentialStoreName, credentialStoreAuthorName, dataChunkSize, isDirectAccess, supportedDocTypes, isRemoteKeyProvisioningSupported) < std::tie(rhs.credentialStoreName, rhs.credentialStoreAuthorName, rhs.dataChunkSize, rhs.isDirectAccess, rhs.supportedDocTypes, rhs.isRemoteKeyProvisioningSupported);
  }
  ...
  inline std::string toString() const {
    ...
    os << ", isRemoteKeyProvisioningSupported: " << ::android::internal::ToString(isRemoteKeyProvisioningSupported);
    os << "}";
    return os.str();
  }
};
